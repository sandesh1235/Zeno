...
import { Alert, Dimensions, Modal, Pressable, SafeAreaView, ScrollView, StatusBar, StyleSheet, Switch, Text, TextInput, View } from 'react-native';
import * as Notifications from 'expo-notifications';
import { templates, newExercise, newRoutine, uid, MUSCLE_GROUPS, SET_OPTIONS, REP_OPTIONS, type Exercise, type Routine } from './src/data';
import { defaultState, loadState, saveState, type SavedState, type LoggedSet, type WorkoutLog } from './src/storage';
...
export default function App() {
  ...
  if (active) return <Workout routine={active} unit={state.profile.unit} finish={(volume, records, log) => { update({ completed: state.completed + 1, volume: state.volume + volume, records: { ...state.records, ...records }, history: [log, ...state.history] }); setActive(null); setTab('Progress'); }} cancel={() => setActive(null)} />;
...
function Workout({ routine, unit, finish, cancel }: { routine: Routine; unit: 'kg' | 'lb'; finish: (volume: number, records: Record<string, number>, log: WorkoutLog) => void; cancel: () => void }) { const [sets, setSets] = useState<Record<string, ActiveSet[]>>(() => Object.fromEntries(routine.exercises.map(e => [e.id, Array.from({ length: e.sets }, () => ({ weight: '', reps: '', rpe: '', done: false }))]))); const [notes, setNotes] = useState(''); const [seconds, setSeconds] = useState(0); const [startedAt] = useState(() => Date.now()); useEffect(() => { const id = setInterval(() => setSeconds(x => x + 1), 1000); return () => clearInterval(id); }, []); const edit = (id: string, i: number, field: keyof ActiveSet, value: string | boolean) => setSets(s => ({ ...s, [id]: s[id].map((x, n) => n === i ? { ...x, [field]: value } : x) })); const complete = () => { let volume = 0; const records: Record<string, number> = {}; const loggedSets: LoggedSet[] = []; routine.exercises.forEach(e => sets[e.id].forEach((s, i) => { if (s.done) { const kg = toKg(Number(s.weight) || 0, unit); const reps = Number(s.reps) || 0; volume += kg * reps; records[e.name] = Math.max(records[e.name] || 0, kg); loggedSets.push({ exerciseId: e.id, exerciseName: e.name, muscle: e.muscle, setNumber: i + 1, weightKg: kg, reps, rpe: s.rpe ? Number(s.rpe) : null, notes: '' }); }})); const log: WorkoutLog = { id: `wo-${uid()}`, routineId: routine.id, routineName: routine.name, startedAt, completedAt: Date.now(), durationSeconds: seconds, notes, sets: loggedSets }; finish(volume, records, log); };
